...
install_dependencies(){
  ...
  run "sudo systemd-nspawn -D ${CONTAINER_DIR}/${DISTRIBUTION}_armhf-development apt-get install -y build-essential pkg-config git wget fakeroot kernel-package zlib1g-dev libncurses5-dev u-boot-tools"
}
...
build_linux_kernel(){
  ...
  run "sudo systemd-nspawn -D ${CONTAINER_DIR}/${DISTRIBUTION}_armhf-development /bin/bash -c \"cd /root/linux-sunxi && git checkout experimental/sunxi-3.10\""
