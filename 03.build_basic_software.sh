...
install_oh_my_zsh(){
  ...
  run "sudo systemd-nspawn -D  ${CONTAINER_DIR}/${DISTRIBUTION}_armhf /bin/bash -c \"apt-get install -y zsh git\""
  ...
  run "sudo systemd-nspawn -D  ${CONTAINER_DIR}/${DISTRIBUTION}_armhf /bin/bash -c \"sed -i '/env zsh/c\# env zsh' install.sh\""
  ...
  run "sudo systemd-nspawn -D  ${CONTAINER_DIR}/${DISTRIBUTION}_armhf /bin/bash -c \"sed -i '1s/^/DISABLE_UPDATE_PROMPT=\"true\"\n/' ~/.zshrc\""
  run "sudo systemd-nspawn -D  ${CONTAINER_DIR}/${DISTRIBUTION}_armhf /bin/bash -c \"sed -i '1s/^/DISABLE_AUTO_UPDATE=\"true\"\n/' ~/.zshrc\""
}
